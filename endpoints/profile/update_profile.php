...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['file'])) {
        ...
        $filename = $owner_id . '_' . time() . '.' . pathinfo($file['name'], PATHINFO_EXTENSION);
        $upload_path = $upload_dir . $filename;

        // Move uploaded file
        if (move_uploaded_file($file['tmp_name'], $upload_path)) {
            $query = "UPDATE owner SET image = ? WHERE id = ?";
    ...
    $query = "UPDATE owner SET $field = ? WHERE id = ?";
    $stmt = $conn->prepare($query);
    ...
    $stmt->bind_param("si", $value, $owner_id);
