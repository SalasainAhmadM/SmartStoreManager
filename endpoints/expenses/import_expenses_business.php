    ...
    if (json_last_error() === JSON_ERROR_NONE && is_array($expenseData)) {
        foreach ($expenseData as $expense) {
            ...
            if (!$stmt->execute()) {
                ...
                header("Location: ../../owner/manageexpenses.php?imported=false&error=" . urlencode($stmt->error));
                exit();
            }
        }

        header("Location: ../../owner/manageexpenses.php?imported=true");
        exit();
    } else {
        header("Location: ../../owner/manageexpenses.php?imported=false&error=Invalid data format");
        exit();
    }
} else {
    header("Location: ../../owner/manageexpenses.php?imported=false&error=Invalid request");
    exit();
}
